...
function collectCoins(jet, coin){
    ...
    let xVelocity = Phaser.Math.Between(-100, 100);
    let yVelocity = Phaser.Math.Between(150, 200)
    coin.setVelocity(xVelocity, yVelocity)
    score += 10;
    scoreText.setText('Score : '+ score)
}

function setObjectVelocity(bombs){
    bombs.children.iterate(function(bombs){
        let xVelocity = Phaser.Math.Between(-100, 100);
        let yVelocity = Phaser.Math.Between(150, 200)
        bombs.setVelocity(xVelocity, yVelocity)
    })
}
...
function destroyBomb(ammo, bomb){
    ...
    let xVelocity = Phaser.Math.Between(-100, 100);
    let yVelocity = Phaser.Math.Between(150, 200)
    bomb.setVelocity(xVelocity, yVelocity)
    score += 10;
    scoreText.setText('Score : ' + score);
}
